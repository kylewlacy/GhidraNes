		...
		*/
		...
		for (bank = 0; (bank + 2) * 0x2000 < rom.prgRom.length; bank++) {
			...
			MemoryBlockDescription.initialized(0x8000, 0x2000, "PRG" + bank, romPermissions, rombankBytes, bank != 0, monitor)
				.create(program);
			MemoryBlockDescription.initialized(0xA000, 0x2000, "PRG" + bank + "_MIRROR", romPermissions, rombankBytes, bank != 0, monitor)
				.create(program);
		}
		...
		byte[] rombankBytes = Arrays.copyOfRange(rom.prgRom, bank*0x2000, (bank+1)*0x2000);
		//CPU $C000-$DFFF (or $8000-$9FFF): 8 KB PRG ROM bank, fixed to the second-last bank
		MemoryBlockDescription.initialized(0xC000, 0x2000, "PRG" + bank + "_FIXED1", romPermissions, rombankBytes, false, monitor)
			.create(program);
